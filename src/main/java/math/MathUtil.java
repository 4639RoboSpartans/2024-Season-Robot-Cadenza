...
public class MathUtil {
   ...
   *
   * @return the angle in degrees that the vector &lt;x, y&gt; makes with the positive x-axis,
  ...
  /**
   * @return the angle in degrees that the line y = slope * x makes with the positive x-axis,
   *     measured counterclockwise
   */
  ...
  /**
   * Constrains x to the range [min, max). values outside the range will wrap around. For min = 0,
   * this behaves the same as mod
   *
   * @return a value v such that v is in [min, max) and v = x - N * (max - min) where N is an
   *     integer
   */
